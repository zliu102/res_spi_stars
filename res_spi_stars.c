...
reservoir_sampler_stars(PG_FUNCTION_ARGS)
{
    ...
    TupleDescInitEntry(tupdesc, (AttrNumber) 1, "mjd,linenpixright", INT4OID, -1, 0);
    ...
    for(row = 0; row < SPI_processed; row++){
        ...
        int attnum2 = SPI_fnumber(SPI_tuptable->tupdesc, "linenpixright");
